...
export default function ToolCard({ title, description, href, icon, variant = "default", delay = 0 }: Props) {
  ...
  if (variant === "featured") {
    return (
      ...
      >
        <div className="w-10 h-10 rounded-lg bg-white/20 flex items-center justify-center text-xl mb-4" aria-hidden="true">
          {icon}
        </div>
  ...
  return (
    <a
      href={href}
      className="block p-5 rounded-xl border border-[var(--color-border)] bg-white hover:border-[var(--color-coral-200)] hover:shadow-[0_2px_8px_rgba(232,93,58,0.08)] transition-all duration-200 group"
      style={{ animation: `fade-in-up 0.4s ease-out ${delayMs}ms both` }}
    >
